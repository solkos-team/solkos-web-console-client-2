...
function obtenerFechas(mes, año) {
  ...
  const fechaPrimerDiaMesAnterior = `${añoAnterior}-${String(
    mesAnterior
  ).padStart(2, "0")}-01`;

  return [fechaUltimoDia, fechaPrimerDiaMesAnterior];
}
const updateTelemetriaStatus = (
  dateStat: Date | null | undefined,
  telemetriaStatus,
  setTelemetriaStatus
) => {
  const isDateValid =
...
const obtenerFechaMasReciente = (fechaStrA?: string, fechaStrB?: string) => {
    if (fechaStrA === undefined && fechaStrB === undefined) {
      ...
      if (fechaStrA === undefined) {
        return new Date(fechaStrB!)
      }
      if (fechaStrB === undefined) {
        return new Date(fechaStrA)
      }
      const fechaA = new Date(fechaStrA)
      const fechaB = new Date(fechaStrB)
